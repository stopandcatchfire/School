...
public class Game extends Form implements Runnable {
	...
	public Game(){
			...
			bgs = new BGSound("BGsound.mp3");
			fightSound = new Sound("fightSound.mp3");
			doorOpen = new Sound("doorOpen.mp3");
			spawnAlien = new Sound("spawnAlien.mp3");
			...
			//System.out.println(mv.getHeight() + ", " + mv.getWidth() + ", " + mv.getX() + ", " + mv.getY());
			//gw.setRandLocations();
			...
			heal.getAllStyles().setBgColor(ColorUtil.LTGRAY);
			...
			OpenCommand openDoor = new OpenCommand(gw, doorOpen);
			score.setCommand(openDoor);
			addKeyListener('s', openDoor);
			
			PauseCommand pc = new PauseCommand(this);
			pause.setCommand(pc);
			
			HealCommand hc = new HealCommand(gw);
			heal.setCommand(hc);
			...
			gw.init(mv.getX(), mv.getY(), mv.getWidth(), mv.getHeight(),fightSound,spawnAlien);
			this.isSound(gw.getSound());
